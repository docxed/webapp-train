...
if (isset($_POST['register'])) {
    ...
    $phone = $_POST['phone'];
    $avatar = $_FILES['avatar']; // รูปภาพจะถูกส่งมาในรูปแบบของ array
    $password = $_POST['password'];

    $allow = array('jpg', 'jpeg', 'png'); // กำหนดไฟล์ที่อนุญาตให้อัปโหลด
    $extention = explode(".", $avatar['name']); // แยกชื่อไฟล์กับนามสกุลออกจากกัน
    ...
    try {
        ...
        if ($check_email->rowCount() > 0) {
            ...
            if (in_array($fileActExt, $allow)) { // ตรวจสอบว่านามสกุลไฟล์ที่อัปโหลดมาเป็นไฟล์ที่อนุญาตหรือไม่
                if ($avatar['size'] > 0 && $avatar['error'] == 0) { // ตรวจสอบว่าไฟล์มีขนาดมากกว่า 0 และไม่มี error
                    if (move_uploaded_file($avatar['tmp_name'], $filePath)) { // อัปโหลดไฟล์
                        // ทำการเพิ่มข้อมูลลงใน Database เพื่อสมัครสมาชิก
                        $sql = $conn->prepare("INSERT INTO users (user_email, user_firstname, user_lastname, user_sex, user_born, user_department, user_phone, user_avatar, user_password) VALUES (:email, :firstname, :lastname, :sex, :born, :department, :phone, :avatar, :password)");
    ...
    try {
        ...
        if ($check_email->rowCount() < 1) { // เคส ไม่เจออีเมลใน Database
            ...
            $_SESSION['mySex']  = $users['user_sex'];
            $_SESSION['myBorn'] = $users['user_born'];
            $_SESSION['myDepartment'] = $users['user_department'];
            $_SESSION['myPhone'] = $users['user_phone'];
            $_SESSION['myAvatar'] = $users['user_avatar'];
            $_SESSION['myJobTitle'] = $users['user_jobtitle'];
            if ($users['user_jobtitle'] == 'manager') {
