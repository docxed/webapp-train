...
<?php
$myId = $_SESSION['myId'];
$posts_stmt = $conn->query("SELECT * FROM posts WHERE user_id = $myId ORDER BY post_timestamp DESC");
$posts_stmt->execute();
$posts = $posts_stmt->fetchAll();
?>
<div class="container my-5">
    <?php $nPosts = $conn->query("SELECT COUNT(*) FROM posts WHERE user_id = $myId")->fetchColumn(); ?>
    <p class="h2">รายการโพสต์ของฉันทั้งหมด <span class="badge rounded-pill text-bg-primary"><?= $nPosts; ?></span></p>
    <hr><br>
    <div class="row">
        <p class="text-end">
